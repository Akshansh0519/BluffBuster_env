...
def launch_baseline_stream(limit: int, max_new_tokens: int):
    ...
    def _run():
        ...
        try:
            ...
            print(f"[baseline] Loading Qwen/Qwen2.5-7B-Instruct (4-bit)...")
            t0 = time.time()
                ...
                try:
                    ...
                    if not repo_id:
                        try:
                            ...
                            repo_id = f"{username}/bluffbuster-checkpoints"
                        except Exception:
                            repo_id = "bluffbuster-checkpoints"
                    try:
    ...
    with gr.Blocks(title="BluffBuster GRPO Fix") as app:
        ...
        with gr.Tab("Train (GPU)"):
            gr.Markdown(
                ...
                "|--------|-------|----------|------------------|-----------|-------|\n"
                "| DEBUG     | Qwen 7B 4-bit | 20  | ~15 min  | ~$0.27  | Smoke test only |\n"
                "| DEMO_FAST | Qwen 7B 4-bit | 200 | ~1.5 hr  | ~$1.60  | ✅ Recommended if time is short |\n"
                "| FULL_FAST | Qwen 7B 4-bit | 200 | ~3 hr    | ~$3.20  | ✅ Full evidence, deadline-safe |\n"
                "| DEMO      | Qwen 7B 4-bit | 200 | ~2 hr    | ~$2.10  | Higher quality, more time |\n"
                "| FULL      | Qwen 7B 4-bit | 300 | ~2.5 hr  | ~$2.65  | Validator-tuned full run |\n"
            )
            cfg_dd    = gr.Dropdown(
        ...
        # ── Tab 5: Baseline (GPU) — untrained 7B baseline runner ──
        with gr.Tab("Baseline (GPU)"):
            gr.Markdown(
                "## Untrained 7B Baseline (the LLM 'before' row)\n\n"
                "Runs **the same Qwen2.5-7B-Instruct, the same prompt builder, "
                ...
                "| Knob | Default | Why |\n"
                "|------|---------|-----|\n"
                "| `limit` (eval seeds) | 15 | Subset of the full 50-seed "
                "suite (seed-matched, never reordered). |\n"
                ...
                "| model | Qwen2.5-7B-Instruct | Same as DEMO_FAST / "
                "FULL_FAST / FULL training configs. |\n"
            ...
            bl_out = gr.Textbox(
                ...
                placeholder=(
                    ...
                    "Phases:\n"
                    "  1. Load Qwen2.5-7B-Instruct (4-bit, ~3-4 min)\n"
                    "  2. Run frozen eval on N seeds (~5-8 min)\n"
