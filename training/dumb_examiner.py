...
Scientific design (the "right" before/after baseline):
  Same architecture       — Qwen2.5-7B-Instruct (matches all training configs
                            on main; 1.5B was removed by the validator
                            because it caused parse_failure_rate ~1.0)
...
def _extract_first_json(text: str) -> str:
    ...
    The untrained Qwen2.5-Instruct family are chat models — they tend to
        wrap output in markdown fences ("```json ... ```") or prefix it with
        ...
        because GRPO teaches it to emit raw JSON. (Defensive on 7B too —
        7B-Instruct still occasionally adds preamble before the JSON.)
...
def load_dumb_examiner(
    ...
    Args:
        model_name:     HF model id. Defaults to Qwen2.5-7B-Instruct (matches
                        DEMO_FAST / FULL_FAST / FULL configs on main).
        max_seq_length: 1024 — same as DEBUG / DEMO_FAST / FULL_FAST configs.
        ...
        max_new_tokens: Eval-only generation cap. 256 is required because
                        ...
                        produced parse_failure_rate=1.0 on 1.5B (kept the
                        defensive headroom on 7B too).
