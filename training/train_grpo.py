...
def train(config: TrainingConfig, eval_config: dict) -> dict:
    ...
    # ── GRPOConfig ────────────────────────────────────────────────────────
    checkpoint_steps = 10
    checkpoint_root = os.path.join("outputs", "checkpoints", config.config_name.lower())
    ...
    # Resume search:
    # 1) config-scoped checkpoints (new layout): outputs/checkpoints/<config>/checkpoint-*
    # 2) legacy root checkpoints (old layout): outputs/checkpoints/checkpoint-*
    # This prevents losing progress after we introduced config namespacing.
    legacy_root = os.path.join("outputs", "checkpoints")
    resume_ckpt = _find_latest_checkpoint(checkpoint_root)
    if resume_ckpt is None:
        resume_ckpt = _find_latest_checkpoint(legacy_root)
    ...
    if resume_ckpt is not None:
        ...
        # If checkpoint already reached/finished the scheduled steps, skip retraining.
        if resume_step is not None and resume_step >= total_steps_estimate:
            print(
                f"[resume] checkpoint step {resume_step} >= planned {total_steps_estimate}; "
                "skipping trainer.train() and proceeding to final eval."
            )
        else:
