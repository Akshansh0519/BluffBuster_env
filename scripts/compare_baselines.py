...
Inputs (any subset; missing files are silently skipped):
    ...
    outputs/eval/dumb_baseline_metrics.json
        Single metrics dict (untrained 7B, no LoRA).

    outputs/eval/final_metrics.json
        Single metrics dict (GRPO-trained 7B, with LoRA).
...
_DISPLAY_ORDER = [
    "RandomExaminer",
    "BayesianHeuristicExaminer",
    "DumbLLM (7B, no training)",
    "TrainedLLM (7B + GRPO LoRA)",
]
...
def _collect() -> dict[str, dict]:
    ...
    if dumb:
        out["DumbLLM (7B, no training)"] = dumb

    trained = _safe_load(os.path.join(_EVAL_DIR, "final_metrics.json"))
    if trained:
        out["TrainedLLM (7B + GRPO LoRA)"] = trained
...
def _build_bar_chart(collected: dict[str, dict], output_path: str) -> str:
    ...
    short_names = {
        ...
        "DumbLLM (7B, no training)":         "Dumb LLM\n(7B)",
        "TrainedLLM (7B + GRPO LoRA)":       "Trained\n(GRPO)",
    }
    ...
    color_map = {
        ...
        "DumbLLM (7B, no training)":         "#f0a868",
        "TrainedLLM (7B + GRPO LoRA)":       "#3aa755",
    }
