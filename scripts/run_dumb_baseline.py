"""
scripts/run_dumb_baseline.py — Evaluate the UNTRAINED Qwen2.5-7B-Instruct
...
Speed defaults
--------------
    --limit 15            # 15 seeds (subset of full 50)
    --max_new_tokens 256  # CLASSIFY JSON for 10 sections + chat preamble
    --max_seq_length 1024 # matches DEMO_FAST / FULL_FAST / DEBUG

Estimated wall-clock on A10G (7B 4-bit):  model load ~3-4 min  +
                               ~2.5 s/turn * 15 seeds * ~4 turns
                               = ~5 min eval; total ~10 min.

Usage
-----
    # default — fast 15-seed run on 7B
    python scripts/run_dumb_baseline.py

    # full 50-seed run (~25-30 min on A10G)
...
HF Hub upload (auto)
--------------------
If `HF_TOKEN` env var is set, the metrics JSON is also pushed to a
checkpoint repo. The repo id is, in order of preference:
  1. $CHECKPOINT_REPO env var  (explicit override)
  2. "<your-hf-username>/bluffbuster-checkpoints"  (auto-detected via
     HfApi.whoami() — same pattern as training/train_grpo.py on main)
  3. "bluffbuster-checkpoints"  (last-resort fallback)
The metrics land at `dumb_baseline/dumb_baseline_metrics.json` and survive
...
def _print_summary(metrics: dict, output_path: str) -> None:
    print("\n" + "=" * 60)
    print("UNTRAINED 7B BASELINE — frozen eval suite")
    print("=" * 60)
    headline = [
...
def _resolve_checkpoint_repo(token: str) -> str:
    """Resolve the HF Hub checkpoint repo id.

    Mirrors the pattern in training/train_grpo.py on main (commit 81c5ca8):
      1. $CHECKPOINT_REPO env var  (explicit override wins)
      2. "<hf-username>/bluffbuster-checkpoints"  (auto from HfApi.whoami)
      3. "bluffbuster-checkpoints"  (fallback if whoami fails)
    """
    repo_id = os.environ.get("CHECKPOINT_REPO", "")
    if repo_id:
        return repo_id
    try:
        from huggingface_hub import HfApi
        username = HfApi(token=token).whoami()["name"]
        return f"{username}/bluffbuster-checkpoints"
    except Exception as e:
        print(f"[dumb_baseline] Could not detect HF username ({e!r}); "
              f"falling back to 'bluffbuster-checkpoints'.")
        return "bluffbuster-checkpoints"
...
def _maybe_upload_to_hub(local_path: str) -> None:
    ...
    if not token:
        ...
        return
    repo_id = _resolve_checkpoint_repo(token)
    try:
        ...
        api.upload_file(
            ...
            repo_type="model",
            token=token,
            commit_message="dumb baseline metrics (untrained 7B)",
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run untrained 7B baseline on the frozen eval suite."
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1024,
        help="Same default as DEMO_FAST / FULL_FAST / DEBUG.max_seq_length.",
    )
    parser.add_argument(
        "--max_new_tokens",
        type=int,
        default=256,
        help=("Generation cap per examiner turn. 256 fits a valid CLASSIFY "
              "JSON for 10 sections (~85 tokens) plus a chat preamble; "
              "64 was empirically too short and produced parse_failure=1.0."),
    )
    parser.add_argument(
        "--no_4bit",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=15,
        help=(
            "Use only the first N seeds. Default 15 = quick subset; "
            "set 50 for the full eval suite."
        ),
    )
    parser.add_argument(
        "--output",
